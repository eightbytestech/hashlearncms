...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.courseChapter.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.course-chapters.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('chapter_group') ? 'is-invalid' : '' }}" name="chapter_group_id" id="chapter_group_id">
                    @foreach($chapter_groups as $id => $chapter_group)
                        <option value="{{ $id }}" {{ old('chapter_group_id') == $id ? 'selected' : '' }}>{{ $chapter_group }}</option>
                    @endforeach
                </select>
